...
import {StyleSheet, View, SafeAreaView, ImageBackground,Text,TextInput,TouchableOpacity} from 'react-native';
...
const Login = props => {
  ...
  const setLogin=async()=>{
    ...
    if(flag==1 && loginData.length>0){
      ...
      console.log('flag:',flag,typeof(errorFlag),typeof(initialValues), errorFlag===initialValues)
  ...
  const validate = async () => {
    ...
    await Joi.validate(validateValues, validationSchema.loginSchema, error => {
     ...
     let value= await(validation(validateValues,validationSchema.loginSchema));
